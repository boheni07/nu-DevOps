    ...
    <div className="w-full max-w-[1600px] mx-auto space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="text-[12px] font-black text-indigo-500 uppercase tracking-widest mb-1">Active Project</div>
        ...
        <div className="flex items-center gap-10 text-right">
          <div>
            <div className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-1">Overall Progress</div>
            <div className="text-3xl font-black text-indigo-600">{stats.overallProgress}%</div>
          </div>
          <div className="w-px h-12 bg-slate-100"></div>
          <div>
            <div className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-1">Project Status</div>
            <div className={`text-sm font-black px-4 py-1.5 rounded-full mt-1 inline-block ${currentProject.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-700'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-slate-400 text-sm font-bold mt-1">S-Curve Analysis</p>
            </div>
            <div className="flex gap-6 text-[11px] font-black text-slate-400 uppercase tracking-widest">
              <span className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-indigo-50 shadow-md shadow-indigo-100"></span> ACTUAL</span>
        ...
        <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col">
          ...
          <p className="text-slate-400 text-sm font-bold mb-8 uppercase">Distribution</p>
          <div className="flex-1 flex flex-col items-center justify-center">
            <div className="h-52 w-52 relative">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[12px] text-slate-400 font-black uppercase tracking-widest mt-1">DONE</span>
              </div>
            </div>
...
const StatCard = ({ title, value, color, icon }: any) => (
  <div className="bg-white p-6 rounded-[1.5rem] border border-slate-200 shadow-sm group hover:border-indigo-300 transition-all hover:shadow-lg">
    <div className="flex justify-between items-start mb-4">
      <span className="text-[12px] font-black text-slate-400 uppercase tracking-widest">{title}</span>
      <span className="text-xl bg-slate-50 w-8 h-8 flex items-center justify-center rounded-xl">{icon}</span>
    </div>
...
const LegendItem = ({ label, color, value }: any) => (
  <div className="flex items-center justify-between py-2 border-b border-slate-50 last:border-none">
    ...
    <span className="text-sm font-black text-slate-800">{value} <span className="text-[11px] text-slate-400 ml-0.5">ITEMS</span></span>
  </div>
);
