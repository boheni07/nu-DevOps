          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-3">
                ...
                {(generatedReport.status === 'Submitted' || generatedReport.status === 'Approved') && (
                  <div className="flex items-center gap-2 px-6 py-2.5 bg-slate-50 rounded-xl border border-slate-100">
                    <span className="w-2 h-2 rounded-full bg-indigo-500 animate-pulse"></span>
                    <span className="text-[10px] font-black text-indigo-600 uppercase tracking-widest">보고 완료됨 (수정 불가)</span>
                  </div>
                )}
              </div>
            </div>
            <div className="p-8 bg-slate-50/30">
              <textarea
                className={`w-full h-40 border border-slate-200 rounded-3xl p-6 text-sm font-medium text-slate-700 outline-none transition-all resize-none shadow-inner leading-relaxed ${generatedReport.status === 'Draft'
