...
const MyTasks: React.FC<MyTasksProps> = ({ tasks, setTasks, currentUser, projects }) => {
  ...
  const myTasks = useMemo(() => {
    ...
    // 3. 최하위 레벨(Leaf Task)만 필터링: 현재 id가 다른 태스크의 parentId로 쓰이지 않는 업무만 포함
    filtered = filtered.filter(t => !tasks.some(allT => allT.parentId === t.id));
        ...
        {columns.map(col => {
          ...
          return (
            <div
              ...
              className={`flex flex-col h-full rounded-[2rem] border-2 p-4 transition-all duration-300 overflow-hidden ${isOver ? `bg-indigo-50/40 ${col.accent} scale-[1.01]` : 'bg-slate-100/40 border-transparent'
                }`}
            >
                ...
                {colTasks.map(task => {
                  ...
                  return (
                    <div
                      ...
                      className={`group relative bg-white p-3.5 rounded-2xl border transition-all duration-200 cursor-pointer active:cursor-grabbing ${isBeingDragged ? 'opacity-20 shadow-none border-slate-100' :
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center mb-2.5">
                        <span className="text-[9px] font-black text-white bg-indigo-600 px-2 py-0.5 rounded-full uppercase tracking-wider shadow-sm flex items-center gap-1.5 shrink-0">
