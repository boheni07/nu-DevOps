...
### ProgressReporting.tsx
- **보고 프로세스**: 일간/주간/월간 리포트 생성 및 저장, 제출(`Submit`), PM 승인(`Approval`)의 전체 워크플로우를 담당합니다.
- **데이터 집계**: 선택된 기간의 업무 로그(`WorkLog`)를 자동으로 수집하여 테이블 형식으로 렌더링합니다.
- **수정 제한 정책**: `Submitted` 또는 `Approved` 상태인 보고서는 읽기 전용으로 전환되어 데이터의 무결성을 보장합니다.
- **통합 이력 뷰**: 별도의 페이지 이동 없이 하단 섹션에서 타입별 과거 기록에 즉시 접근할 수 있도록 설계되었습니다.
